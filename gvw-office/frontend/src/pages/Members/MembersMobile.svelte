                ...
                {#if membersStore.display.length !== 0}
                    {#each membersStore.display as member (member.id)}
                        <button
                            ...
                            <div class="flex flex-col items-start justify-between mr-auto max-w-3/4 min-w-0 overflow-hidden">
                                <p class="text-gv-dark-text text-dt-7 text-left truncate w-full">{`${member.name} ${member.surname}`}</p>
                                <div class="flex items-center justify-start gap-2  min-w-0 overflow-hidden">
                                    <span class="material-symbols-rounded text-icon-dt-7 text-gv-dark-turquoise  shrink-0">mail</span>
                                    <p class="text-dt-8 text-gv-dark-turquoise truncate min-w-0 w-full">{member.email}</p>
                                </div>
                            </div>
