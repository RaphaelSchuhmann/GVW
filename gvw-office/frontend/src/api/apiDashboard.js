 ...
 *
 * Sends a GET request to `/dashboard/user` endpoint with an Authorization header.
 *
 ...
 *
 * Sends a GET request to `/dashboard/admin` endpoint with an Authorization header.
 *
 * @returns {Promise<{ resp: Response | null, body: any | null }>}
 * An object containing the raw fetch Response (`resp`) and the parsed
 * JSON response body (`body`). Returns `{ resp: null, body: null }`
 * if the request fails before a response is received.
 *
 * The response body may include complete admin dashboard data
 *
 * @example
 * const { resp, body } = await apiGetAdminDashboardData();
 *
 * if (resp?.ok) {
 *   console.log("Admin dashboard data: ", body);
 * }
 */
