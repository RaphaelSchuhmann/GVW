 ...
 *
 * <p>Used for deterministic hashing of values where the original value
 * should not be stored directly. The generated hashes are represented as
...
public class HashUtil {
   ...
   *
   * <p>The input is encoded using UTF-8 before hashing. The resulting digest
   * is returned as a lowercase hexadecimal string.
   *
   * @param value value to hash
   * @return SHA-256 hash as hexadecimal string, or {@code null} if the input
   *         value is null
   *
   ...
   *
   * <p>The input value is hashed using the same SHA-256 algorithm and compared
   * against the stored hash using a constant-time comparison to reduce the
   * risk of timing attacks.
   *
   * @param rawInput unhashed input value
   * @param storedHash previously generated SHA-256 hash
   * @return {@code true} if the generated hash matches the stored hash,
