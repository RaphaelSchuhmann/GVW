...
public class TextEditorService {
  ...
  private String editorAssetsDir;

  public TextEditorService(FileUtils fileUtils) {
  ...
  public AttachmentResource getAssetFile(String filename) {
    ...
    Path filePath =
        fileUtils.resolveFile(
            filename,
            editorAssetsDir,
            ErrorDomain.TEXT_EDITOR,
            ErrorAction.UTILITY,
            ErrorResource.TEXT_EDITOR_CONTENT);
    ...
    try {
      for (MultipartFile file : files) {
        Optional<StoredFile> storedFile =
            fileUtils.storeFile(file, editorAssetsDir, ErrorDomain.TEXT_EDITOR, action);
      ...
      physicalPaths.forEach(fileUtils::deleteFile);
  ...
  public void synchronizeBlockAssets(
    ...
    for (String deadFile : oldIds) {
      fileUtils.deleteFile(deadFile, editorAssetsDir);
    }
  }
   ...
   */
  public void purgeAllBlockAssets(List<TextEditorBlock> blocks) {
    extractFileIds(blocks).forEach(file -> fileUtils.deleteFile(file, editorAssetsDir));
  }
